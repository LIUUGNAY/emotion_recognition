...
for i in range(0, 32):
    process_file = 'D:/python/预处理数据集/data_EMD_python/'
    if i < 9:
    ...
    for j in range(0, 40):
        ...
        dirPath = "D:/python/预处理数据集/EMDData_PLV_python/" + process_name + "/"
        if j < 9:
            pathname = "s0" + str(j + 1) + ".dat"
        else:
            pathname = "s" + str(j + 1) + ".dat"

        dic = {'EMDPLV_Theta': array_plv_Theta, 'EMDPLV_Alpha': array_plv_Alpha, 'EMDPLV_Beta': array_plv_Beta, 'EMDPLV_Gamma': array_plv_Gamma, 'labels': labels}
        createFile()
